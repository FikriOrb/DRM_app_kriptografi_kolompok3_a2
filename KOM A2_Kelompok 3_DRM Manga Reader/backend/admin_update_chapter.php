<?php
declare(strict_types=1);

require_once __DIR__ . '/config.php';

header('Content-Type: application/json; charset=utf-8');
apply_cors_headers();

try {
    ensure_storage_dirs();

    if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
        http_response_code(204);
        exit;
    }

    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        fail('POST required.', 405);
    }

    $token = $_SERVER['HTTP_X_ADMIN_TOKEN'] ?? ($_POST['admin_token'] ?? '');
    if (!hash_equals(ADMIN_UPLOAD_TOKEN, (string) $token)) {
        fail('Invalid admin token.', 403);
    }

    $chapterId = (int) ($_POST['chapter_id'] ?? 0);
    $chapterNumber = (float) ($_POST['chapter_number'] ?? 0);
    $chapterTitle = trim((string) ($_POST['chapter_title'] ?? ''));
    $deletedPageIds = $_POST['deleted_page_ids'] ?? ''; // comma separated list of IDs

    if ($chapterId <= 0) {
        fail('Chapter ID is required.');
    }

    $pdo = pdo();
    $pdo->beginTransaction();

    // Verify chapter exists
    $stmt = $pdo->prepare('SELECT comic_id FROM chapters WHERE id = :id');
    $stmt->execute([':id' => $chapterId]);
    $chapter = $stmt->fetch();
    if (!$chapter) {
        fail('Chapter not found.');
    }

    // Update Title and Number
    if ($chapterTitle !== '' || $chapterNumber > 0) {
        $stmt = $pdo->prepare('UPDATE chapters SET title = :title, number = :number WHERE id = :id');
        $stmt->execute([':title' => $chapterTitle, ':number' => $chapterNumber, ':id' => $chapterId]);
    }

    // Handle Deletions
    if ($deletedPageIds !== '') {
        $idsToDelete = array_filter(array_map('intval', explode(',', $deletedPageIds)));
        if (!empty($idsToDelete)) {
            $placeholders = implode(',', array_fill(0, count($idsToDelete), '?'));
            
            // Delete actual files first
            $stmt = $pdo->prepare("SELECT encrypted_path FROM chapter_pages WHERE id IN ($placeholders) AND chapter_id = ?");
            $params = array_values($idsToDelete);
            $params[] = $chapterId;
            $stmt->execute($params);
            
            foreach ($stmt->fetchAll() as $row) {
                $filePath = __DIR__ . '/' . $row['encrypted_path'];
                if (is_file($filePath)) {
                    @unlink($filePath);
                }
            }

            // Delete from DB
            $delStmt = $pdo->prepare("DELETE FROM chapter_pages WHERE id IN ($placeholders) AND chapter_id = ?");
            $delStmt->execute($params);
        }
    }

    // Handle New Pages (Append)
    if (isset($_FILES['new_pages']) && is_array($_FILES['new_pages']['tmp_name'])) {
        // Get current max index
        $stmt = $pdo->prepare('SELECT MAX(page_index) as max_idx FROM chapter_pages WHERE chapter_id = :chapter_id');
        $stmt->execute([':chapter_id' => $chapterId]);
        $maxIdx = (int) $stmt->fetchColumn();
        $nextIdx = $maxIdx + 1;

        $pageCount = count($_FILES['new_pages']['tmp_name']);
        for ($i = 0; $i < $pageCount; $i++) {
            if (!is_uploaded_file($_FILES['new_pages']['tmp_name'][$i])) {
                continue;
            }
            encrypt_page_update_chapter($_FILES['new_pages'], $i, $chapterId, $nextIdx);
            $nextIdx++;
        }
    }

    // Update comic's updated_at
    $pdo->prepare('UPDATE comics SET updated_at = CURRENT_TIMESTAMP WHERE id = :id')->execute([':id' => (int) $chapter['comic_id']]);

    $pdo->commit();
    ok(['updated' => true]);
} catch (Throwable $e) {
    if (pdo()->inTransaction()) pdo()->rollBack();
    error_log($e->getMessage());
    fail('Update chapter failed.', 500);
}

function detect_upload_mime_update_chap(string $tmpName): string
{
    $finfo = new finfo(FILEINFO_MIME_TYPE);
    $mime = $finfo->file($tmpName);
    if (!in_array($mime, ['image/jpeg', 'image/png', 'image/webp'], true)) {
        fail('Only JPEG, PNG, and WEBP images are allowed.');
    }
    return $mime;
}

function encrypt_page_update_chapter(array $files, int $fileIndex, int $chapterId, int $pageIndex): void
{
    $tmpName = $files['tmp_name'][$fileIndex];
    $mime = detect_upload_mime_update_chap($tmpName);
    $plain = file_get_contents($tmpName);
    if ($plain === false) fail('Unable to read uploaded page.');

    $key = random_bytes(32);
    $iv = random_bytes(16);
    $cipher = openssl_encrypt($plain, 'aes-256-cbc', $key, OPENSSL_RAW_DATA, $iv);
    if ($cipher === false) fail('Page encryption failed.');

    $dir = ENCRYPTED_DIR . '/chapter-' . $chapterId;
    if (!is_dir($dir) && !mkdir($dir, 0750, true) && !is_dir($dir)) {
        fail('Unable to create chapter storage.');
    }

    $fileName = 'page-' . str_pad((string) $pageIndex, 4, '0', STR_PAD_LEFT) . '-' . bin2hex(random_bytes(8)) . '.bin';
    $absolutePath = $dir . '/' . $fileName;
    if (file_put_contents($absolutePath, $cipher, LOCK_EX) === false) {
        fail('Unable to write encrypted page.');
    }

    $relativePath = 'storage/encrypted/chapter-' . $chapterId . '/' . $fileName;
    $stmt = pdo()->prepare('INSERT INTO chapter_pages
        (chapter_id, page_index, encrypted_path, mime_type, byte_size, sha256, key_base64, iv_base64)
        VALUES (:chapter_id, :page_index, :encrypted_path, :mime_type, :byte_size, :sha256, :key_base64, :iv_base64)');
    $stmt->execute([
        ':chapter_id' => $chapterId,
        ':page_index' => $pageIndex,
        ':encrypted_path' => $relativePath,
        ':mime_type' => $mime,
        ':byte_size' => strlen($cipher),
        ':sha256' => hash('sha256', $cipher),
        ':key_base64' => base64_encode($key),
        ':iv_base64' => base64_encode($iv),
    ]);
}
