...
export default function EditComic() {
  ...
  useEffect(() => {
    const fetchComic = async () => {
      try {
        const res = await fetch(`http://localhost/manga_api/api.php?action=comic&id=${id}`);
        ...
        if (result.ok && result.data) {
          ...
          const chapRes = await fetch(`http://localhost/manga_api/api.php?action=chapters&comic_id=${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost/manga_api/admin_update_comic.php', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDeleteChapter = async (chapterId: string) => {
    ...
    try {
      const token = localStorage.getItem('admin_token') || '';
      const res = await fetch(`http://localhost/manga_api/admin_delete_chapter.php`, {
        method: 'POST',
  ...
  const handleAddChapterSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost/manga_api/admin_add_chapter.php', {
        method: 'POST',
        body: formData,
      });
